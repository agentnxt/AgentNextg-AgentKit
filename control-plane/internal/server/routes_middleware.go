...
func (s *AgentFieldServer) applyGlobalMiddleware() {
	...
	var oidcConfig *middleware.OIDCAuthConfig
	if s.config.API.Auth.OIDC.Enabled && (s.config.API.Auth.OIDC.Issuer != "" || s.config.API.Auth.OIDC.JWKSURL != "") {
		validator, err := middleware.NewOIDCValidator(
			context.Background(),
			s.config.API.Auth.OIDC.Issuer,
			s.config.API.Auth.OIDC.JWKSURL,
			s.config.API.Auth.OIDC.Audience,
			s.config.API.Auth.OIDC.AgentIDClaim,
		)
		if err != nil {
			logger.Logger.Error().Err(err).Msg("failed to initialize OIDC validator")
		} else {
			oidcConfig = &middleware.OIDCAuthConfig{
				Enabled:    true,
				AdminScope: s.config.API.Auth.OIDC.AdminScope,
				Validator:  validator,
			}
			logger.Logger.Info().Msg("🔑 OIDC bearer-token authentication enabled")
		}
	}
