...
export function AuthGuard({ children }: { children: React.ReactNode }) {
  const oidcEnabled = Boolean(import.meta.env.VITE_OIDC_ISSUER && import.meta.env.VITE_OIDC_CLIENT_ID);

  if (oidcEnabled) {
    return <OidcAuthGuard>{children}</OidcAuthGuard>;
  }

  return <ApiKeyAuthGuard>{children}</ApiKeyAuthGuard>;
}

function ApiKeyAuthGuard({ children }: { children: React.ReactNode }) {
...
function OidcAuthGuard({ children }: { children: React.ReactNode }) {
  const { apiKey, setApiKey, isAuthenticated, authRequired, clearAuth } = useAuth();
  const oidc = useOidcAuth();
  const [error, setError] = useState("");
  const callbackPath = `${(import.meta.env.VITE_BASE_PATH || "/ui").replace(/\/$/, "")}/callback`;
  ...
  useEffect(() => {
    ...
    const syncToken = async () => {
      try {
        ...
        console.error("Failed to sync OIDC access token", syncError);
        setError("Signed in, but failed to get an API access token.");
      }
    };

    void syncToken();
  }, [oidc.isAuthenticated, oidc.user?.access_token, setApiKey]);
  ...
  if (oidc.isAuthenticated && !apiKey) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-3 text-muted-foreground">
          <Loader2 className="size-5 animate-spin" />
          <span className="text-sm">Finishing OIDC sign-in…</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <Card variant="surface">
          <CardHeader className="space-y-1 pb-4">
            <CardTitle className="text-base">Sign in with SSO</CardTitle>
            <CardDescription>
              Authenticate with your OIDC provider to access the control plane.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button type="button" className="w-full" onClick={() => void handleOidcSignIn()}>
              Connect with SSO
            </Button>

            {oidc.isAuthenticated && (
              <Button type="button" variant="outline" className="w-full" onClick={() => void handleOidcSignOut()}>
                Sign out
              </Button>
            )}
          </CardContent>
