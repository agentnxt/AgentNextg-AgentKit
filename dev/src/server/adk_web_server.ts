...
export class AdkWebServer {
  ...
  private readonly logtoDocumentApiBaseUrl?: string;
  private readonly logtoAccessToken?: string;
  ...
  constructor(options: ServerOptions) {
    ...
    this.logtoDocumentApiBaseUrl = options.logtoDocumentApiBaseUrl;
    this.logtoAccessToken = options.logtoAccessToken;
  ...
  private init() {
    ...
    app.get(
      '/apps/:appName/users/:userId/sessions/:sessionId/documents/verification',
      async (req: Request, res: Response) => {
        try {
          const appName = req.params['appName'];
          const userId = req.params['userId'];
          ...
          const documents =
            (session.state['documentsVerification'] as Record<
              string,
    ...
    app.post(
      '/apps/:appName/users/:userId/sessions/:sessionId/documents/:documentType/verification',
      async (req: Request, res: Response) => {
        try {
          const appName = req.params['appName'];
          const userId = req.params['userId'];
          ...
          const body = req.body as DocumentVerificationPayload;

          if (!DOCUMENT_TYPES.includes(documentType)) {
            res.status(400).json({
              error: `Unsupported document type: ${documentType}. Supported values: ${DOCUMENT_TYPES.join(', ')}`,
            });
            return;
          }
          ...
          if (!body?.status || !DOCUMENT_STATUSES.includes(body.status)) {
            res.status(400).json({
              error: `Invalid verification status. Supported values: ${DOCUMENT_STATUSES.join(', ')}`,
            });
            return;
          }
          ...
          const documentsVerification = {
            ...currentDocuments,
            [documentType]: verificationRecord,
          };
          ...
          await this.persistDocumentVerificationToLogto({
            appName,
            userId,
            sessionId,
            documentsVerification,
...
+-----------------------------------------------------------------------------+`);
  ...
  private async persistDocumentVerificationToLogto({
    appName,
    userId,
    sessionId,
    documentsVerification,
    ...
    userId: string;
    sessionId: string;
    documentsVerification: Record<string, unknown>;
  }): Promise<void> {
    if (!this.logtoDocumentApiBaseUrl) {
      return;
    }

    const response = await fetch(
      `${this.logtoDocumentApiBaseUrl.replace(/\/$/, '')}/document-verification`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(this.logtoAccessToken
            ? {Authorization: `Bearer ${this.logtoAccessToken}`}
            : {}),
        },
        body: JSON.stringify({
          appName,
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to persist document verification to Logto API: ${response.status} ${response.statusText}`,
      );
    }
  }
}
