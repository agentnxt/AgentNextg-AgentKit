...
describe('AdkWebServer', () => {
  ...
  describe('Document verification', () => {
    it('should save and return document verification status', async () => {
      await sessionService.createSession({
        appName: 'testApp',
        userId: 'testUser',
        sessionId: 'sessionId',
      ...
      const upsertResponse = await client.post<{
        documentType: string;
        documentsVerification: Record<string, unknown>;
      }>(
        '/apps/testApp/users/testUser/sessions/sessionId/documents/pan/verification',
        {
          status: 'verified',
      ...
      expect(upsertResponse.data?.documentType).toBe('pan');
      expect(upsertResponse.data?.documentsVerification['pan']).toMatchObject({
        status: 'verified',
        verificationId: 'verif-123',
      });
      ...
      const getResponse = await client.get<{
        ...
        '/apps/testApp/users/testUser/sessions/sessionId/documents/verification',
      );
      expect(getResponse.status).toBe(200);
      expect(getResponse.data?.documents['pan']).toMatchObject({
        status: 'verified',
        verificationId: 'verif-123',
      });
    });

    it('should call Logto document API when configured', async () => {
      ...
      const logtoServer = new AdkWebServer({
        agentLoader,
        sessionService,
        memoryService,
        artifactService,
        port: 1237,
        logtoDocumentApiBaseUrl: 'https://logto.example.com',
        logtoAccessToken: 'token-123',
      });
      const logtoClient = new MockHttpClient(logtoServer.app);

      await sessionService.createSession({
        appName: 'testApp',
        userId: 'testUser',
        sessionId: 'logtoSessionId',
      });

      const response = await logtoClient.post(
        '/apps/testApp/users/testUser/sessions/logtoSessionId/documents/aadhaar/verification',
        {status: 'pending'},
      );
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'https://logto.example.com/document-verification',
