...
type AuthConfig struct {
	...
	// OIDC enables bearer/JWT auth via a standards-compliant OpenID Connect provider.
	OIDC OIDCAuthConfig `yaml:"oidc" mapstructure:"oidc"`
}

// OIDCAuthConfig holds optional OIDC validation settings.
type OIDCAuthConfig struct {
	Enabled    bool   `yaml:"enabled" mapstructure:"enabled"`
	Issuer     string `yaml:"issuer" mapstructure:"issuer"`
	JWKSURL    string `yaml:"jwks_url" mapstructure:"jwks_url"`
	Audience   string `yaml:"audience" mapstructure:"audience"`
	AdminScope string `yaml:"admin_scope" mapstructure:"admin_scope"`
	AgentIDClaim string `yaml:"agent_id_claim" mapstructure:"agent_id_claim"`
}
...
func ApplyEnvOverrides(cfg *Config) {
	...
	if issuer := os.Getenv("AGENTFIELD_API_AUTH_OIDC_ISSUER"); issuer != "" {
		cfg.API.Auth.OIDC.Issuer = issuer
	}
	if jwksURL := os.Getenv("AGENTFIELD_API_AUTH_OIDC_JWKS_URL"); jwksURL != "" {
		cfg.API.Auth.OIDC.JWKSURL = jwksURL
	}
	if audience := os.Getenv("AGENTFIELD_API_AUTH_OIDC_AUDIENCE"); audience != "" {
		cfg.API.Auth.OIDC.Audience = audience
	}
	if adminScope := os.Getenv("AGENTFIELD_API_AUTH_OIDC_ADMIN_SCOPE"); adminScope != "" {
		cfg.API.Auth.OIDC.AdminScope = adminScope
	}
	if agentIDClaim := os.Getenv("AGENTFIELD_API_AUTH_OIDC_AGENT_ID_CLAIM"); agentIDClaim != "" {
		cfg.API.Auth.OIDC.AgentIDClaim = agentIDClaim
	}
	if enabled := os.Getenv("AGENTFIELD_API_AUTH_OIDC_ENABLED"); enabled != "" {
		cfg.API.Auth.OIDC.Enabled = strings.EqualFold(enabled, "true") || enabled == "1"
	}
